...
var curDay = new Date(curDate.toDateString() + " 00:00:00");
...
function ckcPerson(name, depart, birthMonth, birthDay){
    this.name = name;
    this.depart = depart;
    this.birthday = new Date(curDate.getFullYear().toString() + "-" + birthMonth + "-" + birthDay + " 00:00:00");
}
...
for(var i = 0; i < allPerson.length; i++){
    var restDay = getDaysDif(curDay, allPerson[i].birthday);
    if(restDay >= 0 && restDay < 15){
        outputBirth("距 " + allPerson[i].depart + " " + allPerson[i].name + " 的生日还有 " + restDay + "天！");
    }
}
